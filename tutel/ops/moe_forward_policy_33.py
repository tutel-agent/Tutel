...
@torch.compile
def silu_mul(x, scal_factor=1):
    return torch.nn.functional.silu(x.narrow(-1, 0, x.size(-1) // 2)) * x.narrow(-1, x.size(-1) // 2, x.size(-1) // 2) * scal_factor

def moe_forward(x, topk_ids, topk_weights, B, B_post, C_prev, C, fused_sum=True, layer_info=(0, 1)):
    ...
    config = {'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 4, 'num_warps': 2, 'num_stages': 2, 'waves_per_eu': 0}
    ...
    if fused_sum:
        B_post_act = silu_mul(B_post_out, 0.5)
        config = {'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 1, 'num_warps': 4, 'num_stages': 1, 'waves_per_eu': 2}
        ...
        out = ops.glu_expert_bf16xf8_down_noshared(C_prev_out, topk_ids, topk_weights, C, C.scale_inv)
        return out
    else:
        ...
        config = {'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 1, 'num_warps': 2, 'num_stages': 1, 'waves_per_eu': 2}
