...
def ref_moe_full(x, topk_ids, topk_weights, B, B_post, C_prev, C, shared_B_bf16, shared_C_bf16, **kwargs):
    ...
    shared_out = torch.nn.functional.silu(shared_out.narrow(-1, 0, shared_out.size(-1) // 2)) * shared_out.narrow(-1, shared_out.size(-1) // 2, shared_out.size(-1) // 2)
    ...
    return shared_out + (C_out.view(*topk_ids.shape, -1) * topk_weights.unsqueeze(-1)).sum(dim=1)
...
def main():
    ...
    try:
        ...
        from tutel.ops.moe_forward_policy_33_shared import moe_forward as opt_moe_full

    opt_moe_full(A, topk_ids, topk_weights, B, B_post, C_prev, C, shared_B_bf16, shared_C_bf16)
    opt_moe_full(A, topk_ids, topk_weights, B, B_post, C_prev, C, shared_B_bf16, shared_C_bf16)
    opt_moe_full(A, topk_ids, topk_weights, B, B_post, C_prev, C, shared_B_bf16, shared_C_bf16)
    ...
    with profile(activities=[ProfilerActivity.CUDA], record_shapes=True) as prof:
      with record_function("model_inference"):
        for i in range(100):
          L = opt_moe_full(A, topk_ids, topk_weights, B, B_post, C_prev, C, shared_B_bf16, shared_C_bf16)
    print(prof.key_averages().table(sort_by="cpu_time_total", row_limit=100))

    if batch <= 3200:
      R = ref_moe_full(A, topk_ids, topk_weights, B, B_post, C_prev, C, shared_B_bf16, shared_C_bf16)
      compare(L, R)
